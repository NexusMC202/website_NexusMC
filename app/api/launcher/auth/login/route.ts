...
import { ensureAuthTables, hashLauncherToken, hashPassword } from "../../../../../db/auth";
...
export async function POST(request: Request) {
  ...
  const user = await env.DB.prepare(
    "SELECT id,email,minecraft_nick,password_hash,password_salt,skin_key FROM users WHERE lower(email)=? OR lower(minecraft_nick)=? LIMIT 1",
