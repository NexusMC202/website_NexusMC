...
async function findSession(request: Request) {
  ...
  const row = await env.DB.prepare(
    "SELECT sessions.remaining_entries, sessions.expires_at, users.email, users.minecraft_nick, users.skin_key, users.skin_model FROM sessions JOIN users ON users.id=sessions.user_id WHERE sessions.id=?",
...
export async function GET(request: Request) {
  ...
  return Response.json({
    ...
    user: { email: row.email, minecraftNick: row.minecraft_nick, skinUrl: row.skin_key ? "/api/profile/skin" : null, skinModel: row.skin_model ?? "default" },
  });
}
...
export async function POST(request: Request) {
  ...
  return Response.json({ authenticated: true, remainingEntries: remaining, user: { email: row.email, minecraftNick: row.minecraft_nick, skinUrl: row.skin_key ? "/api/profile/skin" : null, skinModel: row.skin_model ?? "default" } });
}
